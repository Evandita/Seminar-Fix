...
\textbf{Kata kunci:} Multi-Agent Systems, Adversarial Attacks, Large Language Models, Reinforcement Learning, Recursive Worm, LeakAgent, Keamanan Siber, Ekstraksi Kekayaan Intelektual.
\end{abstract}
\end{flushleft}
...
\textbf{Keywords:} Multi-Agent Systems, Adversarial Attacks, Large Language Models, Reinforcement Learning, Recursive Worm, LeakAgent, Cybersecurity, Intellectual Property Extraction.
\end{abstract}
\end{flushleft}
...
\begin{enumerate}
    ...
    Melakukan eksperimen komprehensif pada tiga kategori lingkungan, yakni dataset sintetis untuk validasi internal dengan \textit{ground truth} yang diketahui, aplikasi \textit{honeypot} pada platform komersial (Coze, CrewAI) untuk pengujian kondisi \textit{real-world}, dan aplikasi produktif dari GPT Store untuk validasi transferabilitas \textit{in the wild}. Evaluasi menggunakan metrik kuantitatif meliputi \textit{Attack Success Rate} (ASR), \textit{Topology Fidelity} ($GS_{topo}$), dan \textit{Extraction Efficiency}, dengan perbandingan terhadap metode \textit{baseline} seperti MASLEAK dan PromptFuzz untuk mengukur peningkatan performa yang dicapai.\cite{ref8}
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Mengidentifikasi kerentanan kritis dalam arsitektur MAS komersial yang mengandalkan \textit{security by obscurity}, memberikan bukti empiris (\textit{proof-of-concept}) yang dapat mendorong pengembang platform seperti CrewAI, AutoGen, dan Coze untuk mengimplementasikan mekanisme pertahanan yang lebih \textit{robust}.
...
\begin{enumerate}
    ...
    Bab ini mendeskripsikan metodologi PTRW yang terdiri dari Fase I (\textit{Warhead Generation}) menggunakan PPO dan Fase II (\textit{Delivery System}) dengan mekanisme \textit{iterative peeling} dan \textit{Client-Side Memory}. Protokol evaluasi meliputi pengujian pada dataset MASD, \textit{honeypots} komersial, dan GPT Store dengan metrik kuantitatif serta validasi statistik.
\end{enumerate}
...
Dalam konteks kerangka kerja PTRW, PPO berfungsi sebagai komponen "\textit{warhead generation}" yang merekonseptualisasi ekstraksi IP MAS sebagai serangkaian "\textit{Stateless Unit Tests}" yang diorkestrasi secara eksternal. Pendekatan ini mengatasi keterbatasan fundamental dari metode konvensional yang sering kali gagal akibat kompleksitas ruang keadaan dan kelangkaan sinyal \textit{reward} pada pengaturan \textit{black-box}.\cite{ref1}
...
\begin{enumerate}
    ...
    \textit{LeakAgent} memperkenalkan penggunaan \textit{Reinforcement Learning} dengan PPO untuk menghasilkan \textit{prompt adversarial} optimal terhadap \textit{single-agent LLM}. Kerangka kerja ini menggunakan fungsi \textit{reward} WES yang padat dan \textit{dynamic temperature adjustment} untuk menghasilkan \textit{prompt} yang efektif dan beragam.\cite{ref3} \textit{LeakAgent} berhasil mengatasi keterbatasan \textit{fuzzing} acak dengan memberikan \textit{directional gradient} yang mengarahkan pembelajaran menuju \textit{prompt} yang lebih efektif. Namun, \textit{LeakAgent} dirancang untuk serangan \textit{single-agent} dan belum mendukung kompleksitas topologi MAS, tidak memiliki mekanisme untuk navigasi struktur berlapis, dan tidak menyediakan \textit{Client-Side Memory} untuk \textit{tracking state} eksternal. Penelitian ini mengadaptasi dan memperluas \textit{LeakAgent} sebagai komponen "\textit{warhead generation}" dalam PTRW, mengintegrasikannya dengan mekanisme \textit{Iterative Peeling} dan \textit{Client-Side Memory} untuk mengatasi tantangan \textit{multi-agent}.\cite{ref1}
...
Pengujian sistem PTRW dirancang untuk dilakukan pada tiga kategori lingkungan yang berbeda, masing-masing dengan karakteristik dan tujuan evaluasi yang spesifik, untuk memastikan validitas dan \textit{generalizability} hasil penelitian.
...
Kategori kedua melibatkan \textit{deployment} aplikasi \textit{honeypot} di \textit{platform} komersial Coze dan CrewAI. Dalam skenario ini, penelitian akan men-\textit{deploy} 10 \textit{distinct} MAS \textit{applications} dengan kompleksitas yang bervariasi, mencakup aplikasi dengan 3 hingga 6 \textit{agents}, pada \textit{commercial platforms} yang aktif digunakan. \textit{Applications} ini akan dibuat oleh \textit{domain experts} seperti \textit{PhD students} atau \textit{researchers} yang memiliki keahlian di bidang terkait, yang bertindak sebagai "\textit{honeypots}" dengan karakteristik bahwa \textit{ground truth} tentang konfigurasi internal aplikasi sudah \textit{known} oleh peneliti. Skenario ini dirancang untuk menguji PTRW pada \textit{real black-box platforms} yang memiliki \textit{API limitations} aktual, \textit{potential rate limiting}, dan mungkin \textit{defensive mechanisms} yang telah diimplementasikan oleh \textit{platform provider}. Pengujian ini memberikan \textit{insight} tentang seberapa efektif PTRW dapat beroperasi dalam kondisi \textit{real-world} di mana terdapat berbagai \textit{constraints} dan \textit{protections} yang tidak ada dalam \textit{environment} sintetis.

Kategori ketiga fokus pada pengujian transferabilitas lintas model dengan menargetkan 10 \textit{popular multi-agent applications} dari OpenAI GPT \textit{Store} untuk menguji "\textit{in the wild}" transferabilitas. Skenario ini merepresentasikan kondisi paling realistis di mana sistem PTRW diaplikasikan terhadap target yang tidak dikenal sebelumnya, tanpa akses ke \textit{internal configuration} atau \textit{ground truth}. Karena \textit{ground truth hidden} dalam skenario ini, validasi keberhasilan ekstraksi dilakukan melalui \textit{human review} terhadap \textit{extracted prompts} oleh \textit{experts} yang mengevaluasi apakah informasi yang diekstrak \textit{alignment} dengan \textit{observable behavior} dari aplikasi tersebut.\cite{ref1} Jika \textit{extracted system prompt} menjelaskan \textit{behavior} yang konsisten dengan apa yang dapat diamati dari interaksi normal dengan aplikasi, maka ekstraksi dianggap berhasil. Pengujian kategori ini krusial untuk memvalidasi hipotesis transferabilitas bahwa \textit{warheads} yang dilatih pada \textit{proxy model} dapat efektif terhadap berbagai target \textit{production systems} yang menggunakan model dan konfigurasi berbeda.
...
Hipotesis pertama (H1) berkaitan dengan \textit{Topology Advantage}, memprediksi bahwa PTRW akan mencapai \textit{Topology Fidelity} yang lebih tinggi dengan skor $GS_{topo} > 0.90$ dibandingkan dengan MASLEAK, khususnya pada \textit{non-linear topologies} seperti \textit{Star} dan \textit{Random}. \textit{Rationale} di balik hipotesis ini adalah kemampuan fundamental dari pendekatan \textit{iterative peeling} untuk \textit{traverse branches} tanpa mengalami \textit{context loss} yang signifikan. Berbeda dengan MASLEAK yang menggunakan \textit{single-pass traversal} di mana akumulasi \textit{context} menyebabkan degradasi performa pada \textit{node} yang lebih dalam, \textit{iterative peeling} melakukan \textit{reset} terhadap \textit{context noise} di setiap \textit{step} karena setiap ekstraksi dilakukan sebagai \textit{query} independen yang hanya membawa konteks minimal yang diperlukan untuk mencapai target \textit{node} spesifik. Mekanisme ini memungkinkan \textit{accurate mapping} terhadap cabang-cabang paralel dalam topologi kompleks tanpa \textit{interference} dari riwayat percakapan sebelumnya, yang menjadi masalah utama dalam pendekatan \textit{sequential}. Pengujian akan dilakukan dengan membandingkan \textit{graph edit similarity} antara \textit{shadow map} yang dihasilkan PTRW \textit{versus} MASLEAK terhadap \textit{ground truth topology} pada \textit{subset dataset} MASD yang fokus pada topologi \textit{non-linear}.
...
Hipotesis ketiga (H3) berkaitan dengan Cost-Benefit Tradeoff, mengakui bahwa PTRW akan mengincur query cost yang 2-3 kali lebih tinggi dalam hal Extraction Efficiency dibandingkan MASLEAK karena nature dari iterative approach yang memerlukan multiple rounds of probing. Namun, hipotesis memprediksi bahwa tradeoff ini justified karena PTRW akan memiliki "Chain Break" failure rate yang significantly lower, yaitu kurang dari 5\%, dibandingkan dengan MASLEAK yang dapat mencapai failure rate 30-40\% pada topologi kompleks. Chain break terjadi ketika propagasi payload terputus di tengah jalan, menyebabkan loss of semua data yang telah diekstrak sebelumnya. Dengan resilience mechanism yang dimiliki PTRW, termasuk kemampuan retry per-layer dan verification setelah setiap ekstraksi, overall success probability per computational budget sebenarnya lebih baik meskipun per-attempt cost lebih tinggi. Analogi sederhana: lebih baik melakukan 3 attempts yang masing-masing memiliki 90\% success rate (expected successful extractions = 2.7) daripada 1 attempt dengan 40\% success rate (expected = 0.4). Pengujian akan membandingkan total API calls required untuk complete topology extraction versus success rate, kemudian menghitung cost-effectiveness metric sebagai ratio successful extractions per 1000 API calls.

Validasi statistik untuk ketiga hipotesis akan dilakukan menggunakan rigorous statistical tests. Untuk H1 dan H2 yang membandingkan continuous metrics (Topology Fidelity dan ASR), paired t-test akan digunakan untuk comparing means antara PTRW dan baseline methods pada same set of targets, memastikan statistical significance dari perbedaan yang diamati. Untuk H3 yang melibatkan efficiency distributions yang mungkin non-normal, Mann-Whitney U test (non-parametric alternative) akan digunakan untuk membandingkan distribusi query costs dan success rates. Semua tests akan menggunakan significance level $\alpha = 0.05$, dengan additional Bonferroni correction jika diperlukan untuk multiple comparisons. Confidence intervals 95\% akan dihitung untuk semua effect sizes untuk memberikan gambaran tentang magnitude dari improvements yang dihasilkan oleh PTRW.
