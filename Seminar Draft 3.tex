...
\justifying
Lanskap kecerdasan buatan (AI) kontemporer sedang mengalami pergeseran tektonik, bertransisi dari era model bahasa besar (LLM) monolitik yang beroperasi dalam isolasi menuju era Multi-Agent Systems (MAS). Jika LLM generasi sebelumnya dapat dianalogikan sebagai ensiklopedia statis yang sangat cerdas, MAS merepresentasikan sebuah “organisasi digital” yang terdiri dari agen-agen otonom—seperti analis keuangan, pemrogram, dan pengawas etika—yang berkolaborasi melalui protokol terstruktur untuk memecahkan masalah kompleks.\cite{ref1} Platform orkestrasi seperti CrewAI, AutoGen, dan Coze telah mendemokratisasi kemampuan ini, memungkinkan pengembang untuk merancang topologi kognitif yang rumit di mana output dari satu agen menjadi input bagi agen lain, menciptakan kecerdasan yang muncul (emergent intelligence) yang jauh melampaui kemampuan model tunggal.

Dalam paradigma baru ini, nilai ekonomi dan strategis dari sistem AI tidak lagi semata-mata terletak pada bobot model (model weights) atau kode infrastruktur dasar, yang semakin terkomodifikasi dan terbuka (open-source). Sebaliknya, aset paling berharga—dan yang paling rentan—adalah “arsitektur kognitif” dari sistem tersebut. Arsitektur ini mencakup system prompts yang mendefinisikan persona dan batasan perilaku agen, instruksi tugas prosedural yang memandu logika penyelesaian masalah, spesifikasi alat (tool specifications) yang memungkinkan interaksi dengan dunia eksternal, dan topologi orkestrasi yang mengatur aliran informasi dan hierarki keputusan.\cite{ref1} Konfigurasi inilah yang membedakan antara asisten generik dan solusi tingkat perusahaan yang sangat spesifik. Oleh karena itu, kerahasiaan dan integritas komponen-komponen ini merupakan Kekayaan Intelektual (IP) yang krusial.

Namun, mekanisme keamanan yang melindungi aset-aset ini sangat tertinggal dibandingkan laju inovasinya. Praktik industri saat ini sebagian besar bergantung pada model “keamanan kotak hitam” (black-box security), di mana pengembang berasumsi bahwa dengan menyembunyikan konfigurasi internal agen di balik API publik, sistem tersebut aman dari ekstraksi atau manipulasi.\cite{ref1} Sejarah keamanan siber telah berulang kali membuktikan bahwa “keamanan melalui ketidakjelasan” (security by obscurity) adalah strategi pertahanan yang rapuh. Tanpa verifikasi formal atau pertahanan yang sadar akan keadaan (state-aware), sistem ini rentan terhadap serangan adversarial yang dirancang untuk mengeksploitasi perilaku probabilistik LLM.

Tantangan utama dalam mengamankan MAS adalah kompleksitas inherent dari interaksi antar-agen. Pendekatan akademis konvensional sering mencoba memodelkan serangan terhadap sistem ini menggunakan kerangka kerja Multi-Agent Reinforcement Learning (MARL), di mana penyerang dianggap sebagai agen yang belajar dalam lingkungan tersebut.

\subsection*{1.2 Rumusan Masalah}
\addcontentsline{toc}{subsection}{1.2 Rumusan Masalah}
Permasalahan mendasar yang menjadi fokus penelitian ini adalah ketidakmemadainya metodologi serangan dan evaluasi keamanan konvensional dalam menghadapi arsitektur MAS yang terdistribusi dan tertutup (black-box). Secara rinci, rumusan masalah dapat dijabarkan sebagai berikut:

\begin{enumerate}
    \item \textbf{Kegagalan Pendekatan MARL dalam Pengaturan Black-Box:}
    
    Bagaimana mengatasi keterbatasan fundamental Multi-Agent Reinforcement Learning (MARL) dalam skenario serangan kotak hitam, di mana fungsi transisi internal dan status agen perantara tidak dapat diamati? Dalam kondisi ini, penyerang menghadapi masalah credit assignment yang ekstrem, di mana sinyal kegagalan dari agen terakhir tidak memberikan informasi yang cukup untuk memperbaiki strategi serangan pada agen awal, mengakibatkan kegagalan pembelajaran kebijakan serangan yang efektif.\cite{ref1}
    \item \textbf{Inefisiensi Metode Fuzzing dan Heuristik:}
    
    Sejauh mana keterbatasan metode berbasis fuzzing (seperti PromptFuzz) yang mengandalkan mutasi acak atau heuristik tanpa panduan gradient? Dalam ruang pencarian bahasa alami yang berdimensi tinggi, pendekatan ini sering kali mendegradasi menjadi pencarian acak yang tidak efisien secara komputasi, memerlukan ribuan kueri untuk menemukan satu celah, dan menghasilkan prompt yang memiliki transferabilitas rendah antar-model.\cite{ref5}
    \item \textbf{Fragilitas Propagasi dan Degradasi Konteks:} Bagaimana memitigasi fenomena “Lost in the Middle” dan degradasi konteks yang menjadi kelemahan utama serangan tipe worm tunggal (seperti MASLEAK)? Saat serangan mencoba melintasi rantai agen yang panjang secara sekuensial, akumulasi riwayat percakapan sering kali menyebabkan agen di kedalaman rantai gagal memproses instruksi propagasi atau format data, memutus rantai serangan dan menyebabkan hilangnya seluruh data yang telah diekstrak.\cite{ref1}
    \item \textbf{Transferabilitas Serangan Lintas Model dan Arsitektur:}
    
    Apakah mungkin untuk menghasilkan prompt adversarial yang dilatih pada model proksi open-source (white-box) menggunakan Reinforcement Learning yang dapat mentransfer efektivitasnya untuk mengekstrak IP dari berbagai arsitektur MAS black-box yang tidak diketahui sebelumnya? Hipotesis ini bertumpu pada asumsi adanya keselarasan universal (universal alignment) dalam cara model bahasa diprogram untuk mengikuti instruksi sistem.\cite{ref1}
\end{enumerate}
...
\bibitem{ref1} Transferability of Single-Agent MAS Attacks.pdf
\bibitem{ref2} IP Leakage Attacks Targeting LLM-Based Multi-Agent Systems - arXiv
\bibitem{ref3} RL-based Red-teaming Agent for LLM Privacy Leakage - OpenReview
\bibitem{ref4} RL-based Red-teaming Agent for LLM Privacy Leakage - arXiv
\bibitem{ref5} PROMPTFUZZ: Harnessing Fuzzing Techniques for Robust Testing of Prompt Injection in LLMs - Semantic Scholar API
\bibitem{ref6} PROMPTFUZZ: Harnessing Fuzzing Techniques for Robust Testing of Prompt Injection in LLMs - ResearchGate
\bibitem{ref7} Literature Review: IP Leakage Attacks Targeting LLM-Based Multi-Agent Systems
\bibitem{ref8} IP Leakage Attacks Targeting LLM-Based Multi-Agent Systems | Request PDF
\bibitem{ref9} RL-based Red-teaming Agent for LLM Privacy Leakage - OpenReview
\bibitem{ref10} PROMPTFUZZ: Harnessing Fuzzing Techniques for Robust Testing of Prompt Injection in LLMs - arXiv
\bibitem{ref11} AgentFuzzer: Generic Black-Box Fuzzing for Indirect Prompt Injection against LLM Agents
\end{thebibliography}
