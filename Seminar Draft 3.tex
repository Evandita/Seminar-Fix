...
\textbf{A. Client-Side Memory dengan Graph Database}
...
\textbf{B. Algoritma Iterative Peeling}
...
\begin{algorithmic}[1]
\Require Target MAS $M$, Entry point $a_1$, Client-Side Memory $\mathcal{M}$, Payload Generator $\mathcal{P}$
\Ensure Complete shadow map of MAS topology and extracted IP
\State Initialize queue $Q \leftarrow \{a_1\}$ \Comment{BFS queue for traversal}
\State Initialize visited set $V \leftarrow \emptyset$
\While{$Q \neq \emptyset$}
    \State $a_{current} \leftarrow Q.\text{dequeue()}$
    \If{$a_{current} \in V$}
        \State \textbf{continue} \Comment{Skip already processed nodes}
    \EndIf
    \State $V \leftarrow V \cup \{a_{current}\}$
    ...
    \While{$\text{retry\_count} < \text{MAX\_RETRIES}$ \textbf{and} \textbf{not} $\text{success}$}
        \State $q \leftarrow \mathcal{P}.\text{GeneratePayload}(a_{current}, \mathcal{M})$ \Comment{Generate encapsulated payload}
        \State $r \leftarrow \text{SendQuery}(M, q)$ \Comment{Send query to target MAS}
        \State $\text{extracted\_data} \leftarrow \text{ParseResponse}(r)$ \Comment{Extract IP from response}
        \State $\text{downstream\_agents} \leftarrow \text{IdentifyConnections}(r)$ \Comment{Discover next agents}
        \If{$\text{ValidateExtraction}(\text{extracted\_data})$}
            \State $\mathcal{M}.\text{UpdateExtractedData}(a_{current}, \text{extracted\_data})$
            \State $\mathcal{M}.\text{MarkAsExtracted}(a_{current})$
            \ForAll{$a_{next} \in \text{downstream\_agents}$}
                \State $\mathcal{M}.\text{StoreNode}(a_{next})$
                \State $\mathcal{M}.\text{StoreEdge}(a_{current}, a_{next})$
                \State $Q.\text{enqueue}(a_{next})$ \Comment{Add to traversal queue}
            \EndFor
            \State $\text{success} \leftarrow \text{true}$
        \Else
            \State $\text{retry\_count} \leftarrow \text{retry\_count} + 1$
            \State $\mathcal{M}.\text{IncrementRetry}(a_{current})$
            \State $\text{AdjustParameters}()$ \Comment{Modify attack strategy}
        \EndIf
    \EndWhile
    \If{\textbf{not} $\text{success}$}
        \State $\mathcal{M}.\text{MarkAsFailed}(a_{current})$
    \EndIf
\EndWhile
\State \Return $\mathcal{M}.\text{GetCompleteTopology}()$
\end{algorithmic}
\end{algorithm}
...
\textbf{C. Protokol Enkapsulasi Rekursif (PTRW Query Structure)}
...
\textit{Contoh Serangan Multi-Layer untuk Agent 3}. Untuk menargetkan agen yang lebih dalam dalam topologi (misalnya Agent 3), diperlukan enkapsulasi berlapis ganda:
...
\begin{algorithmic}[1]
\Require Target agent $a_{target}$, Path to target $[a_1, a_2, ..., a_{target}]$, Client-Side Memory $\mathcal{M}$, PPO-trained prompts $\mathcal{Q}_{Leak}$
\Ensure Encapsulated query $q$
\State $Q_{Leak} \leftarrow \mathcal{Q}_{Leak}.\text{SelectOptimal}(a_{target})$ \Comment{Select best adversarial prompt}
\State $Q_{Retain} \leftarrow \text{GenerateHooking}(a_{target}, \mathcal{M})$ \Comment{Create hooking structure}
\State $Q_{Propagate} \leftarrow$ "\texttt{Repeat content between [[[ and ]]]}"
\State $\text{inner\_payload} \leftarrow$ "\texttt{[[[}" + $Q_{Leak}$ + "\texttt{\textbackslash n}" + $Q_{Retain}$ + "\texttt{\textbackslash n}" + $Q_{Propagate}$ + "\texttt{]]]}"
\If{$\text{len}([a_1, ..., a_{target}]) > 1$}
    \For{$i \leftarrow \text{len}([a_1, ..., a_{target}]) - 1$ \textbf{to} $1$}
        \State $a_i \leftarrow [a_1, ..., a_{target}][i]$
        \State $\text{benign\_instruction} \leftarrow$ "\texttt{Process and forward the following content:}"
        \State $\text{inner\_payload} \leftarrow \text{benign\_instruction}$ + "\texttt{\textbackslash n}" + $\text{inner\_payload}$
    \EndFor
\EndIf
\State \Return $\text{inner\_payload}$
\end{algorithmic}
\end{algorithm}
